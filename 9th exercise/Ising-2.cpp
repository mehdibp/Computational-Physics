/*******************************************************************************************************/
/***                                                                                                 ***/
/***  Template for a Ising model code - part 2                                                       ***/
/***                                                                                                 ***/
/***  Ising model - part 2, v1.0; Date: 1399/04/29                                                   ***/
 ...
 * Openfile(), Init(), Execute(), PostProcess(), magnetization(), Susceptibility() functions.
 */
...
float *M;							// magnetization for step i_th.

// All the following parameters are in the reduced unit system
float T, h, J, N;                   // in file.

ofstream snapshot("snapshot(h=0.1).txt", ios::out | ios::trunc);	 	// Keeps model snapshot
ofstream magnet  ("magnet.txt"  , ios::out | ios::trunc);		// save magnetization per step
ofstream suscept ("Susceptibility.txt", ios::out | ios::trunc); // save susceptibility per step
ofstream energy ("Energy.txt", ios::out | ios::trunc);  		// save energy per step
...
float Susceptibility(float );	  // Given the temperature, it gives the susceptibility.
float magnetization();			  // Magnetization per Spin.
float Energy();
...
int main () {
	...
	while (T>0.5) {
    	...
    	Execute();			// 100 step to achieve thermal balance.
    	PostProcess();		// 500 step to calculate the statistical average.
...
void Openfile () {
    ...
    step_N2 = data[5];			// The 5th line in the file is the number to calculate the statistical average.
...
void PostProcess() {
	...
	float X =  0; 						// amount of susceptibility
	float MM = 0;						// amount average magnetization
	float en = 0;
	
	for (int i=0; i<step_N2; i++)
    	M[i] = 0;						// Clears M
...
// this function calculate the susceptibility of the spins ======================
float Susceptibility(float T) {
	
	float average_M  = 0;				// average magnetization      <M>
	float average_M2 = 0;				// Mean square magnetization <M^2>
	
	for(int i=0; i<step_N2; i++)
		average_M += M[i];
		average_M /= (float) step_N2;
		
	for(int i=0; i<step_N2; i++)
		average_M2 += (M[i]*M[i]);
		average_M2 /= (float) step_N2;
		
	float delta_M = average_M2 - (average_M * average_M);	// X = delta_M/T
	
	return delta_M/T;										// Susceptibility
}
